        ...
        <c:when test="${not empty cart and cart.size() > 0}">
            ...
            <div class="cart-table" >
                <c:forEach var="item" items="${cart}">
                    <div class="cart-item">
                        <img src="image/${item.product.id}.jpg" alt="商品画像${item.product.id}" onerror="this.src=('image/syokki_osara.png')">
                    ...
                    <c:set var="total_count" value="${total_count = total_count + item.count }"/>
                    <c:set var="total_price" value="${total_price = total_price + item.product.price * item.count }"/>
                </c:forEach>
            </div>
            <div class="cart-info" >
                ${cart.size()}種類(合計${total_count }個)の商品があります。<br>金額は合計${total_price }円です。
            </div>
        </c:when>
        <c:otherwise>
